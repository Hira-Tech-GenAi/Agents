...
def extract_info_from_cv(text: str) -> Dict[str, Optional[str]]:
    ...
    # Extract skills (looking for common skill section patterns)
...
if st.session_state.download_ready:
    ...
    with col2:
        # Create Excel file
        df = pd.DataFrame({
            'Field': ['Name', 'Email', 'Skills'],
            'Value': [
                application_info['name'],
                application_info['email'],
                application_info['skills']
            ]
        })
        ...
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='Application Info')
            writer.close()
        
        st.download_button(
            label="📊 Download Application Summary (Excel)",
            data=excel_buffer.getvalue(),
            file_name="application_summary.xlsx",
